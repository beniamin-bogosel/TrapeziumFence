...
\begin{description}
  ...
  certificate verifier, and certificate assembler.  The verifier recomputes
  local claims and checks that all leaves tile the original domain by the same
  dyadic splitting rule.  Missing dyadic children fail immediately.  The
  assembler replaces survivor leaves through a refinement chain.
...
tests/test\_cert\_cli.sh & Builds a tiny certificate, verifies it, deletes one leaf, and checks that verification fails quickly with a coverage error. \\
\bottomrule
\end{longtable}
