...
the pairs \((DC,BA)\) and \((CB,AD)\).  A theoretical necessary condition for
...
There are two exact opposite-pair candidate values: \((DC,BA)\) and \((CB,AD)\).  Let the two
supporting line directions make acute angle \(\gamma\in[0,\pi/2]\), and let
\(A_Q=\Area(DCBA)\).  The implementation uses two algebraically identical
formulas.
...
box\_certainly\_inadmissible & Returns true only when a whole box is proved inadmissible.  It checks CCW convexity through cross products, side lengths through squared distances, and optionally the reflection half-domain condition \(a_1+b_1\ge1\) (stored internally as \(c_1+d_1\ge1\)). \\
box\_small\_area\_certifies\_low & Returns true only when the interval upper bound for area satisfies \(A_{\rm ub}\le\theta^2/4\), so the estimate \(L_{DC,BA}/\sqrt {A_Q}\le 2\sqrt {A_Q}\) proves the box below threshold. \\
\bottomrule
\end{longtable}
...
\[
  A^\ast=(0.3582548434,\ 0.7071006812),\qquad
  B^\ast=(0.6417451566,\ 0.7071006812).
\]
...
\[
\begin{aligned}
R_A&=[0.3427734375,\ 0.369140625]\times[0.6865234375,\ 0.7270507812],\\
R_B&=[0.630859375,\ 0.6572265625]\times[0.6865234375,\ 0.7270507812].
\end{aligned}
\]
...
metric used by the analysis script, is \(0.032438555\).
...
\(R_A\times R_B\), the \(A\)- and \(B\)-bounds are farthest-corner distances
from \(A^\ast\) and \(B^\ast\), and the four-dimensional product bound is their
...
Thus there is no possible above-threshold optimizer outside \(R_A\times R_B\).
...
\[
\begin{aligned}
&738{,}288\text{ leaves},\quad 14{,}550\text{ discarded},\quad
488{,}028\text{ certified},\\
&213\text{ flat-area},\quad 224{,}479\text{ nonoptimal},\quad
11{,}018\text{ survivors}.
\end{aligned}
\]
