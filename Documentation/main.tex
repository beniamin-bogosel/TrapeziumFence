...
\title{Interval Verification Documentation for \code{TrapeziumGPT}}
\author{}
\date{July 2026}
...
recorded experiments for the \code{TrapeziumGPT} package.  The package verifies
...
the local leaf claims and the global dyadic tiling of the original domain.
\end{abstract}
...
For a quadrilateral \(Q=ABCD\), the code computes six scale-invariant upper
bounds \(F_i(Q)\) for the normalized shortest fence value:
\[
  f(Q) \le \min_{0\le i<6} F_i(Q).
\]
The six items are four vertex-angle bounds and two opposite-edge-pair bounds.
A box is certified below \(\theta\) when interval arithmetic proves that the
upper endpoint of an enclosure for \(\min_i F_i\) is at most \(\theta\).
...
\begin{description}
  ...
  upper-bound functionals, natural and centered interval enclosures, the
  minimum envelope used by certification, and the optional
  ...
  \item[\code{cert.c}, \code{cert.h}] JSONL certificate writer and stateless
  certificate verifier.  The verifier recomputes local claims and checks that
  all leaves tile the original domain by the same dyadic splitting rule.
...
\begin{enumerate}
  ...
  \([f_{\mathrm{lo}},f_{\mathrm{hi}}]\) for the minimum of the six upper-bound
  items.  If the enclosure is finite and \(f_{\mathrm{hi}}\le\theta\), the leaf
...
The refined output plus the discarded/certified leaves of the previous run
forms a complete certificate for the original root box.
...
The two opposite-pair items correspond to the fence constructions associated to
...
opposite-pair items.  If the finite intervals are disjoint, then no point in
the box can satisfy the equality condition, and the box is written with status
\code{nonoptimal}.
...
upper-bound envelope is too wide to certify \(f(Q)\le\theta\).
...
\subsection{Opposite-Pair Bounds}

There are two opposite-pair items: \((AB,CD)\) and \((BC,DA)\).  Let the two
...
pair\_bound & Static helper implementing the opposite-pair bound.  It selects the apex formula or the parallel-safe formula unless a test forces one. \\
compute\_items & Static builder for the six item jets from the four coordinate jets.  It also constructs the quadrilateral area \(A_Q=(c_2+c_1d_2-d_1c_2)/2\). \\
functionals\_eval & Public routine that encloses all six items on a coordinate box, in natural or centered form. \\
functionals\_pair\_forced & Test hook that evaluates one pair item at a point while forcing the apex or parallel-safe formula. \\
functionals\_min & Encloses the minimum envelope over the six items.  Non-finite item enclosures are ignored; if all six are non-finite, the result is \(+\infty\), so the search cannot certify the box. \\
functionals\_opposite\_pairs\_disjoint & Encloses the two opposite-pair items and returns true only when both finite intervals are disjoint, certifying failure of the pair-equality necessary condition for optimality. \\
\bottomrule
\end{longtable}
...
parse\_line & Static parser for the simple JSONL certificate format. \\
...
find\_leaf & Static binary search for an exact dyadic box in the sorted certificate array. \\
...
verify\_cover\_node & Static recursive tiling audit.  It descends the dyadic tree and requires every generated node to be represented by exactly one certificate leaf or by its two children. \\
...
main & Parses options, dispatches point evaluation, verification, full search, or survivor refinement; writes certificates and prints statistics. \\
\bottomrule
\end{longtable}
...
\begin{description}
  \item[\code{--theta T}] Threshold for certification.
  ...
  \item[\code{--pair-eq-cert}] Certify boxes as \code{nonoptimal} when the two opposite-pair fence intervals are disjoint.
  ...
  \item[\code{--verify FILE}] Re-check a certificate.
  \item[\code{--eval c1 c2 d1 d2}] Print the six items and minimum at a point.
...
\midrule
\endhead
Base & 0.025 & 82.416 & 215758 & 11555 & 116319 & 213 & 17884 & 69787 &
\(1.66385{\times}10^{-2}\) \\
Refine 1 & 0.0125 & 112.479 & 360798 & 2967 & 282735 & 0 & 50065 & 25031 &
\(3.72991{\times}10^{-4}\) \\
Refine 2 & 0.00625 & 35.136 & 119594 & 28 & 51386 & 0 & 55878 & 12302 &
\(1.14571{\times}10^{-5}\) \\
Refine 3 & 0.003125 & 18.052 & 66134 & 0 & 17312 & 0 & 39816 & 9006 &
\(5.24218{\times}10^{-7}\) \\
\bottomrule
\end{longtable}
\endgroup
...
survivor leaves of the preceding run.  To obtain a single auditable certificate
for the whole root domain, one should assemble the chain by replacing survivors
at each stage, then run \code{./fence\_validate --verify} with the same
certificate options and sufficient precision.  The recorded statistics above
are the local statistics of the refinement steps and the postprocessed
survivor analysis.
...
\begin{itemize}
  ...
  the current upper-bound enclosures were not tight enough to certify.
  ...
  Conclusions that use \code{nonoptimal} boxes should be stated as exclusion of
  ...
  threshold, form, half-domain flag, and precision.
